...
func RunAllExamples(symbool string) *market.Candlestick {
	// getCandlestick()
	...
	// getLatestTrade()
	// getHistoricalTrade()
	resp := getLast24hCandlestick(symbool)

	return resp
	// applogger.Info("最新=%v", resp.Close)
}
...
func getLast24hCandlestick(symbol string) *market.Candlestick {
	client := new(client.MarketClient).Init(config.Host)

	resp, err := client.GetLast24hCandlestick(symbol)
